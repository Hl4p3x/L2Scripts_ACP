...
$(function() {
    ...
    /*if(goSubscribe == 'go'){
        var coocieDate = new Date().getTime();
        coocieDate = coocieDate + subscribePeriod*1000;
        var subscribeDate = new Date(coocieDate);
        if($.cookie('subscribe') != "true") {
            $.cookie('subscribe', "true", {
                expires: subscribeDate,
                path: '/'
            });
            setTimeout(function(){
                $('#subscribe-modal').arcticmodal({
    ...
    if(goGift == 'go'){
        setTimeout(function(){
            $('#subscribe-gift-modal').arcticmodal({
                closeOnOverlayClick: false
            });
        },100);
    }*/

    $('.subscribe-modal-buttons .receive').on('click',function(){
        if($('#subscribe-receive').prop("checked")){
            $.ajax({
                url: '/ajax/subscribe',
                type: 'POST',
                ...
                success: function(data){
                    ...
                    if(that.result == true){
                        $('#subscribe-modal').arcticmodal('close');
                        liliumNotify(that.message,'success');
                        $('#mce-EMAIL').val($('.subscribe-modal-input input').val());
            ...
            $('.subscribe-modal-buttons .receive').addClass('notRecive');
            setTimeout(function(){
                $('.subscribe-modal-buttons .receive').removeClass('notRecive');
            }, 600);
        }
    });

    $('.subscribe-modal-buttons .get-gifts').on('click',function(){
        $.ajax({
            url: '/ajax/subscribe',
            type: 'POST',
            data: {request: 'getPrize', char: $('.subscribe-modal-select select').val()},
            beforeSend: function(){
                $('#ajaxLoader').arcticmodal({
                    closeOnEsc: false,
                    closeOnOverlayClick: false
            ...
            success: function(data){
                ...
                var that = $.parseJSON(data);
                if(that.result == true){
                    $('#subscribe-gift-modal').arcticmodal('close');
                    liliumNotify(that.message,'success');
                }else{
                    liliumNotify(that.message,'danger');
                }
